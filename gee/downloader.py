...
import ee
import gee
from .utils import (gen_subcells,
                    download_images_roi,
                    merge_download_dir,
                    merge_download_dir_obsgeo)
from .exceptions import NoEEImageFoundError, EEImageOverlayError, NoEEIntersectionBandsError
...
class GEEDownloader(Downloader):
    ...
    def download_imagecollection(self):
        ...
        for prefix in image_collection_dic:
            ...
            self.info_csv = self.save_dir + '/' + f'{self.aoi_name}_imagecollection_{sensor_type}_info.csv'
    ...
    def __download_imgcoll_assets(self, date, **config):
        ...
        for asset in config:
            ...
            if res!=1:
                continue
            if not hasattr(gee, f'get_descriptions_{asset}'):
                continue
            prefix, acquisition_time, des, des_meta = getattr(gee, f'get_descriptions_{asset}')(temp_dir)
            #
            #
